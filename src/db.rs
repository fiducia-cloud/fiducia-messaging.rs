...
pub async fn apply_schema(pool: &PgPool) -> Result<(), MessagingError> {
    ...
    sqlx::raw_sql(HARDENING_SCHEMA_SQL)
        ...
        .await
        .map_err(MessagingError::database)?;
    sqlx::raw_sql(RETENTION_SCHEMA_SQL)
        .execute(pool)
        .await
        .map_err(MessagingError::database)?;
    Ok(())
...
pub async fn release_outbox_claims(
    ...
    let result = sqlx::query(RELEASE_CLAIMS_SQL)
        .bind(claim_owner)
        .execute(pool)
        .await
        .map_err(MessagingError::database)?;
...
pub async fn inbox_mark_processed(
    ...
    let result = sqlx::query("UPDATE message_inbox SET processed_at = $2 WHERE message_id = $1")
        .bind(message_id)
        .bind(at)
        .execute(pool)
        .await
...
mod tests {
    ...
    fn positive_ms_rejects_zero_and_overflow() {
        assert_eq!(
            positive_ms(Duration::from_millis(1500), "x").unwrap(),
            1500
        );
